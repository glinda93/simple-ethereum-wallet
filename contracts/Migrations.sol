...
contract SimpleEthereumWallet {
    ...
    modifier registerPassword(bytes32 password) {
        if (users[msg.sender].passwordHash != 0) {
            ...
            users[msg.sender].passwordHash = keccak256(
                abi.encodePacked(password, msg.sender)
            );
            _;
        }
    }
    ...
    modifier checkPassword(bytes32 password) {
        require(
            users[msg.sender].passwordHash ==
                keccak256(abi.encodePacked(password, msg.sender)),
    ...
    function deposit(bytes32 password)
        external
        payable
    ...
    function withdraw(bytes32 password, uint256 amount)
        external
        checkPassword(password)
        checkBalance(amount)
    {
    ...
    function transfer(
        ...
        address payee,
        uint256 amount
    ) external checkPassword(password) checkBalance(amount) {
    ...
    function closeService(bytes32 password)
        external
        onlyOwner
        checkPassword(password)
    {
